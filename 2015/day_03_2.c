...
int makeKey(int8_t* pos){
    ...
    if(pos[1]==0) totDigits = 1;

    int _key = abs((int)(pos[0] * pow(10,totDigits+1)) + abs(pos[1]));
...
int main(){
    ...
    int8_t pos[2] = {0,0};
    int8_t rpos[2] = {0,0};//robo-santa
    bool move = true; // true = santa, false = robo-santa
    
    Map(int,bool) map = new_map(int,bool);
    map_insert(map, makeKey(pos), true);
    ...
    while(*input != '\0'){
        ...
        int _key;
        if(move) _key = makeKey(pos);
        else     _key = makeKey(rpos);
        
        if(!map_contains(map, _key)){
            // printf("pos:[%d,%d] char:%c key:%d\n",pos[0],pos[1],*input, _key);
            map_insert(map, _key, true);
            total++;
        }
