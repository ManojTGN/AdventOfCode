...
bool strstartswith(char* src, char* subs){
    while(*subs != '\0' && *src != '\0'){
        if(*subs != *src) return false;
        subs++;src++;
    }
...
void process(int** bulbs, uint16_t start[2], uint16_t end[2], int8_t processType){

    for(int i = start[0]; i <= end[0]; i++){
    for(int j = start[1]; j <= end[1]; j++){
        if(bulbs[i][j]+processType < 0) continue;
        bulbs[i][j] += processType;
    }
    }

}

int getNumber(char* input, uint16_t* output){
    ...
    while(*input != ' ' && *input != '\0'){
        ...
        if(!nextNumber){
            output[0] = (output[0]*10) + (*input - '0');
        }else{
            output[1] = (output[1]*10) + (*input - '0');
        }
