...
uint16_t _c(char* str){
    uint16_t index = 0;

    for(uint8_t i = 0; str[i] != '\0'; i++)
        index = (index*10) + (uint8_t)str[i];
...
int main(void *){
    ...
    for(int i = 0; i < 340; i++){
        ...
        while(token != NULL){
            if(word == 0 && strcmp(token,"NOT")){
                proc_[i].type = NOT;
            }else if(proc_[i].type == NOT && word == 1){
                strcpy(proc_[i].wire1,token);
            }
